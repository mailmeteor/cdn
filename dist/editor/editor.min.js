...
goog.editor.Field.prototype.dispatchChange=function(a){this.startChangeEvents(!0,a)};
goog.editor.Field.prototype.handleChange=function(){if(!this.isEventStopped(goog.editor.Field.EventType.CHANGE)){var a=this.getField_();if(a){var b=function(a,b){return a.replace(b,"")},c=a.innerHTML,d=/<a[^>]*href="(https?:\/\/[^"]+\/){{[^"]+}}"/g;d.test(c)&&(b=c.replace(d,b),goog.editor.node.replaceInnerHtml(a,b))}this.changeTimerGecko_&&this.changeTimerGecko_.stop();this.isEverModified_=this.isModified_=!0;this.isEventStopped(goog.editor.Field.EventType.DELAYEDCHANGE)||this.delayedChangeTimer_.start()}};
...
goog.editor.plugins.ImageHandler.prototype.safeInsertPlaceholder_=function(a){a&&a.id&&a["data-src"]&&(this.safeInsertImage_({id:a&&a.id,width:a&&a.width,height:a&&a.height,name:a&&a.name,src:"data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7","data-src":a["data-src"]}),a=this.getImageById(a.id))&&(a.style.background='#EFF1F2 url("https://ssl.gstatic.com/ui/v1/icons/mail/inline_image_placeholder.png") center center no-repeat',a.style.backgroundSize="contains",a.style.borderRadius=
